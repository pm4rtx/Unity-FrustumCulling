...
nasm -fwin64 -Oxv ../Sources/%file_name%.asm -o %file_name%_null.obj -l %file_name%_null.lst > nul

:: SSE2 version
set fname=%file_name%_aosoa4_aabb_stream_simd4_sse2
nasm -DFUNCTION_NAME=%fname% -fwin64 -Oxv -o %fname%.obj -l %fname%.lst ../Sources/%file_name%.asm > nul
...
nasm -DFUNCTION_NAME=%fname% -DUSE_AVX=1 -fwin64 -Oxv -o %fname%.obj -l %fname%.lst ../Sources/%file_name%.asm > nul
...
nasm -DFUNCTION_NAME=%fname% -DUSE_AVX=1 -DSIMD_SIZE=4 -fwin64 -Oxv -o %fname%.obj -l %fname%.lst ../Sources/%file_name%.asm > nul
...
nasm -DFUNCTION_NAME=%fname% -DUSE_AVX=1 -DUSE_AOSOA4_INPUT=1 -fwin64 -Oxv -o %fname%.obj -l %fname%.lst ../Sources/%file_name%.asm > nul
...
nasm -DFUNCTION_NAME=%fname% -DUSE_EXTRATESTS=1 -fwin64 -Oxv -o %fname%.obj -l %fname%.lst ../Sources/%file_name%.asm > nul

set fname=%file_name%_extratests_aosoa8_aabb_stream_simd8_avx
nasm -DFUNCTION_NAME=%fname% -DUSE_EXTRATESTS=1 -DUSE_AVX=1 -fwin64 -Oxv -o %fname%.obj -l %fname%.lst ../Sources/%file_name%.asm > nul

set fname=%file_name%_extratests_aosoa4_aabb_stream_simd4_avx
nasm -DFUNCTION_NAME=%fname% -DUSE_EXTRATESTS=1 -DUSE_AVX=1 -DSIMD_SIZE=4 -fwin64 -Oxv -o %fname%.obj -l %fname%.lst ../Sources/%file_name%.asm > nul

set fname=%file_name%_extratests_aosoa4_aabb_stream_simd8_avx
nasm -DFUNCTION_NAME=%fname% -DUSE_EXTRATESTS=1 -DUSE_AVX=1 -DUSE_AOSOA4_INPUT=1 -fwin64 -Oxv -o %fname%.obj -l %fname%.lst ../Sources/%file_name%.asm > nul

lib /wx /nologo /subsystem:windows /machine:x64 /out:lib%file_name%.lib^
...
link /nologo /opt:ref /dll /map /LARGEADDRESSAWARE /entry:_DllMainCRTStartup /machine:x64 /pdbstripped:%file_name%.pdb /out:%file_name%.dll^
    %file_name%_aosoa4_aabb_stream_simd4_sse2.obj^
    %file_name%_aosoa8_aabb_stream_simd8_avx.obj^
    %file_name%_aosoa4_aabb_stream_simd4_avx.obj^
    %file_name%_aosoa4_aabb_stream_simd8_avx.obj^
